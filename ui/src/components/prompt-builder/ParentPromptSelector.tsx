...
export function ParentPromptSelector({
  ...
  const fetchPrompts = useCallback(async () => {
    ...
    try {
      ...
      const filtered = response.prompts.filter((p) => !excludeIds.includes(p.id))
      setPrompts(filtered)
              ...
              {prompts.map((prompt, index) => (
                ...
                >
                  <div className="flex items-baseline justify-between gap-4 mb-1">
                    <span className="font-medium truncate">{prompt.title || 'Untitled'}</span>
                    <span className="text-xs text-dim shrink-0">{formatDate(prompt.updatedAt)}</span>
                  </div>

                  <div className="flex items-center justify-between gap-4">
                    <span className="text-sm text-dim">{prompt.projectName}</span>
                  </div>
                </div>
              ))}
            </div>
