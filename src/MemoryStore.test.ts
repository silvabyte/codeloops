...
describe('MemoryStore', () => {
  ...
  describe('query', () => {
    beforeEach(async () => {
      // Add test entries
      await store.append({ content: 'Alpha entry', project: 'project-a', tags: ['important'] });
      await store.append({ content: 'Beta entry', project: 'project-a', tags: ['test'] });
      await store.append({
        content: 'Gamma entry',
        project: 'project-b',
        tags: ['important', 'test'],
    ...
    it('should combine filters', async () => {
      const results = await store.query({ project: 'project-b', tags: ['important'] });
      expect(results).toHaveLength(1);
      expect(results[0].content).toBe('Gamma entry');
    });
  });
  ...
  describe('forget', () => {
    ...
    it('should not affect other entries when deleting', async () => {
      const entry1 = await store.append({ content: 'Keep me', project: 'test' });
      const entry2 = await store.append({ content: 'Delete me', project: 'test' });
      const entry3 = await store.append({ content: 'Keep me too', project: 'test' });
